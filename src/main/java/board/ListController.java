...
public class ListController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		// 게시물의 갯수를 카운트한다. 검색어가 있다면 where절이 추가된다.
		int totalCount = dao.selectCount(map);
		
		/* 페이지 처리 start */
		/*
		 모델2 방식의 게시판에서는 요청을 서블릿이 먼저 받으므로 내장객체 사용을
		 위해서 반드시 아래와 같이 메서드를 통해 객체를 얻어온 후 사용해야한다.
		 */
		...
		// 포워드
		req.getRequestDispatcher("/vsCode/board-list.jsp").forward(req, resp);
	}
